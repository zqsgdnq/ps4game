...
void Ps4AudioEmitter::Update(PS4AudioSource*s, SceAudio3dPortId& port) {
	...
	const int frameGranularity = SAMPLE_GRANULARITY;// * currentSound->GetChannels();

	__attribute__((aligned(64))) int16_t iSampleBuffer[frameGranularity];
	...
	sPcm.uiNumSamples	= frameGranularity;

	sAttributes[4].uiAttributeId = SCE_AUDIO3D_ATTRIBUTE_PCM;
	sAttributes[4].pValue		 = &sPcm;
	sAttributes[4].szValue		 = sizeof(sPcm);

	SampleFromSound(currentSound, iSampleBuffer , frameGranularity, samplesUsed);
	samplesUsed += frameGranularity;
